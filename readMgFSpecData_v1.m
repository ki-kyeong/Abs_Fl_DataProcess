function results = readMgFSpecData_v1(name, freq_i, freq_f, freq_step,...
    iter, normmode, plotmode)

% update log
% data2spec_v7에서 넘어와서 MgF spectroscopy용 data를 읽게 만들었음. 
% 주파수를 IR과 UV를 둘다 계산하게 해놓았고,
% plotspec_v3를 만들어서 IR이나 UV regime에서 그림을 그릴 수 있게 하였음.
...
for j = 1 : dfsize
    SumAbsDatas(j,:) = sum(results.AN(results.baselinerange+round(10/results.dt):end,:,j)); % ablation 이후 10 µs 부터
...
results.UV = results.IRAbsfreq*2;

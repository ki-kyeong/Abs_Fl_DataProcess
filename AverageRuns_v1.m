function result = AverageRuns_v1(data, runs)

result.savename = data(runs(1)).savename+"_merge";
result.baselineidx = data(runs(1)).baselineidx;
result.dt = data(runs(1)).dt;
result.freq = data(runs(1)).freq;
...
for i = 1: length(runs)
    if i == 1
        result.abs.norm(:,:,(1:data(runs(i)).size.iter),:) = data(runs(i)).abs.norm(:,:,:,:);
    else
        result.abs.norm(:,:,end+(1:data(runs(i)).size.iter),:) = data(runs(i)).abs.norm(:,:,:,:);
    end
end
...
for i = 1:length(runs)
    if i == 1
        result.fl.norm(:,:,(1:data(runs(i)).size.iter),:) = data(runs(i)).fl.norm(:,:,:,:);
    else
        result.fl.norm(:,:,end+(1:data(runs(i)).size.iter),:) = data(runs(i)).fl.norm(:,:,:,:);
    end
end
